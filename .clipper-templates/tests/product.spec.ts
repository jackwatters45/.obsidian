...
const TEST_URL = "https://www.rei.com/product/255385/merrell-moab-speed-2-gore-tex-hiking-shoes-mens";
const template = loadTemplate("product-clipper");

test.describe("Product Clipper", () => {
  test("extracts all template properties", async ({ page }) => {
    // Serve local HTML fixture
    await page.route(TEST_URL, async (route) => {
      const filePath = path.join(__dirname, "fixtures", "product.html");
