...
const TEST_URL = "https://www.linkedin.com/in/example/";
const template = loadTemplate("linkedin-profile-clipper");

test.describe("LinkedIn Profile Clipper", () => {
  test("extracts all template properties", async ({ page }) => {
    // Serve local HTML fixture
    await page.route(TEST_URL, async (route) => {
      const filePath = path.join(__dirname, "fixtures", "linkedin-profile.html");
